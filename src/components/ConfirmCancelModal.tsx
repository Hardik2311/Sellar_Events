...
import { X, Ban, Mail, Phone, Ticket } from 'lucide-react';
...
const ConfirmCancelModal: React.FC<ConfirmCancelModalProps> = ({ attendee, onConfirm, onCancel }) => {
      ...
      <div className="w-full max-w-sm rounded-lg bg-white dark:bg-[#1E293B] shadow-xl overflow-hidden">
        <div className="flex items-center justify-between border-b border-gray-200 dark:border-slate-700 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-slate-900 dark:text-white">Cancel ticket?</h2>
          </div>
        ...
        <div className="px-4 py-4 space-y-3">
          ...
          <div className="rounded-sm bg-red-50 border border-red-200 px-3 py-2 text-xs text-red-600">
            This will mark the ticket as cancelled. You can revive it later from the attendee list.
          </div>
        </div>

        <div className="flex gap-2 border-t border-gray-200 dark:border-slate-700 px-4 py-3">
          <button onClick={onCancel} className="flex-1 rounded-sm border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
            Go back
          </button>
          <button onClick={onConfirm} className="flex-1 rounded-sm bg-[#FF3B30] px-3 py-2 text-sm font-semibold text-white hover:opacity-90 transition-opacity">
            Yes, cancel ticket
          </button>
        </div>
      </div>
