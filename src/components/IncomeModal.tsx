...
export const IncomeModal = ({ isOpen, onClose, events = [], eventsLoading = false, onSave, preselectedEventId = null }: Props) => {
  const { user, profile } = useAuth();
  const currentUserName = profile?.fullName || user?.displayName || 'Unknown User';
    ...
    <div className="fixed inset-0 z-[8000] flex items-center justify-center bg-black/40 px-4">
      <div className="bg-white dark:bg-[#1E293B] w-full max-w-sm rounded-sm shadow-xl p-5 border border-slate-200 dark:border-slate-800">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Add Income</h2>
        ...
        <label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">Source</label>
        <input
          type="text"
          placeholder="e.g. Sponsor Name, Ticket Bonus"
          value={source}
        ...
        <input
          type="text"
          placeholder="e.g. Sponsorship for stage setup"
          value={description}
