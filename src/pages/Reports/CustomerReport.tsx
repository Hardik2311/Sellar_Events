...
const CustomerReportPageInner: React.FC = () => {
  ...
  const {
    ...
    filtered, summary,
  } = useCustomerReport(profile?.companyId, eventId);
  ...
  const downloadAsPdf = () => {
    ...
    try {
      ...
      autoTable(doc, {
        startY: 38,
        head: [['CUSTOMER', 'PHONE', 'TICKETS', 'TOTAL SPENT (₹)']],
        body: filtered.map(c => [
          c.name,
          c.phone,
          String(c.ticketsBought),
        ...
        columnStyles: { 0: { cellWidth: 'auto' }, 1: { cellWidth: 36 }, 2: { halign: 'center', cellWidth: 26 }, 3: { halign: 'right', cellWidth: 42 } },
        didDrawPage: () => {
  ...
  const downloadAsExcel = () => {
    ...
    try {
      ...
      aoa[footerRow] = ['TOTAL', `${summary.totalCustomers} customers`, '', '', filtered.reduce((s, c) => s + c.ticketsBought, 0), summary.totalRevenue];
      ...
      filtered.forEach((_, idx) => {
        ...
        for (let ci = 0; ci < colCount; ci++) {
          ...
          styleCell(addr, s({ sz: 9, color: { rgb: '1E293B' } }, solidFill(isAlt ? 'F8FAFC' : 'FFFFFF'), { horizontal: ci === 1 || ci === 2 || ci === 3 ? 'left' : 'center', vertical: 'center' }, bblr));
        ...
        <div className="w-full max-w-6xl mx-auto space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 bg-white dark:bg-[#1E293B] p-4 rounded-sm shadow-sm border border-slate-200 dark:border-slate-800">
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Customers</p>
              <p className="text-xl font-extrabold text-slate-900 dark:text-white mt-1">{summary.totalCustomers}</p>
            </div>
            <div className="flex-1 bg-white dark:bg-[#1E293B] p-4 rounded-sm shadow-sm border border-slate-200 dark:border-slate-800">
                ...
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-700">
                    {([
                      ...
                      { key: 'ticketsBought', label: 'tickets' },
                  ...
                  {!selectedEventId ? (
                    <tr>
                      <td colSpan={4} className="text-center py-10 text-slate-400 dark:text-slate-500">
                        Select an event above to view its customers.
                      </td>
                    </tr>
                  ) : loading ? (
                    <tr>
                    ...
                    <tr>
                      <td colSpan={4} className="text-center py-10 text-slate-400 dark:text-slate-500">
                        No customers found for selected period.
                      </td>
                    </tr>
                  ) : filtered.map((c, i) => (
                    <tr key={c.key} className={i % 2 === 0 ? 'bg-white dark:bg-[#1E293B]' : 'bg-slate-50 dark:bg-[#182234]'}>
                      <td className="px-4 py-3">
                        <p className="font-medium text-slate-800 dark:text-slate-200">{c.name}</p>
                        <p className="text-xs text-slate-500 dark:text-slate-400">{c.phone !== '—' ? c.phone : c.email}</p>
                      </td>
                      <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{c.ticketsBought}</td>
                      <td className="px-4 py-3 font-semibold text-slate-900 dark:text-white">₹{c.totalSpent.toLocaleString('en-IN')}</td>
                      <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{formatDate(c.lastPurchase)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
