...
const ExpenseReportPageInner: React.FC = () => {
    const { profile } = useAuth();

    // --- event picker (reuses existing EventListCard) ---
    ...
    useEffect(() => {
        const load = async () => {
            ...
            try {
                ...
                console.error('Failed to load events for expense report', e);
            } finally {
    ...
    const { expenses, loading, addExpense, deleteExpense } = useExpenses(profile?.companyId, selectedEventId ?? undefined);

    // --- filters (shared EventDateFilter context) ---
    ...
    const { filtered, summary } = useMemo(() => {
        if (!appliedFilters) return { filtered: [] as Expense[], summary: { total: 0, count: 0 } };

        let list = expenses.filter(e => e.date >= appliedFilters.start && e.date <= appliedFilters.end);
        if (searchQuery) {
        ...
        <div className="flex min-h-screen flex-col bg-slate-100 dark:bg-[#0F172A] text-[#111827] dark:text-[#F8FAFC] transition-colors duration-200 pb-16">
            ...
            <div className="flex-1 p-4 sm:p-6 lg:p-8">
                <div className="w-full max-w-6xl mx-auto space-y-4">
                    ...
                    {/* Download choice inline dropdown */}
                    {isDownloadOpen && (
                    ...
                    {deleteConfirm && (
                        <div className="fixed inset-0 z-[8500] flex items-center justify-center bg-black/40 px-4">
                            <div className="bg-white dark:bg-[#1E293B] w-full max-w-xs rounded-sm shadow-xl p-5 border border-slate-200 dark:border-slate-800 text-center">
                                <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 mb-4">Delete this expense?</p>
                                <div className="flex gap-2">
                                    <button
                                    ...
                                    <button
                                        onClick={async () => {
                                            if (profile?.companyId && selectedEventId) {
                                                await deleteExpense(profile.companyId, selectedEventId, deleteConfirm);
                                            }
                    ...
                    <ExpenseModal
                        ...
                        onSave={async data => {
                            ...
                            await addExpense(profile.companyId, selectedEventId, data);
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {!selectedEventId ? (
                                        ...
                                        <tr key={exp.id} className={i % 2 === 0 ? 'bg-white dark:bg-[#1E293B]' : 'bg-slate-50 dark:bg-[#182234]'}>
                                            <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{formatDate(exp.date)}</td>
