...
const IncomeReportPageInner: React.FC = () => {
    ...
    const downloadAsPdf = () => {
        ...
        try {
            ...
            doc.text(orgName || 'Event Income Report', pw / 2, 13, { align: 'center' });

            // Title + subtitle below the strip
            doc.setFontSize(18); doc.setFont('helvetica', 'bold'); doc.setTextColor(17, 24, 39);
            doc.text(`Event Income Report — ${eventName}`, 14, 34);
    ...
    const downloadAsExcel = () => {
        ...
        try {
            ...
            aoa[0][0] = orgName || 'Event Income Report';
            aoa[1][0] = `Event Income Report — ${eventName}`;
            const periodLabel = isAllTime
                ? 'All Time'
                : `${formatDate(appliedFilters.start)} → ${formatDate(appliedFilters.end)}`;
            aoa[2][0] = `Generated: ${formatDate(Date.now())}   |   Period: ${periodLabel}`;
            aoa[4][0] = 'SUMMARY';
            aoa[5][0] = `Total Income: ₹${summary.total.toLocaleString('en-IN', { minimumFractionDigits: 2 })}   |   Entries: ${summary.count}`;
